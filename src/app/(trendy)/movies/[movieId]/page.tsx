...
const MovieId = ({ params }: params) => {
  ...
  const {
    ...
    error: detailsErrMsg,
  } = detailsQuery(movieId, "movie");
  const {
    isLoading: credIsLoading,
    data: credits,
    isError: credErrState,
    error: credErr,
  } = creditsQuery(movieId, "movie");
  const {
    isLoading: recommendIsLoading,
    data: recommendations,
    isError: reccomErrorState,
    error: reccomErr,
  } = recommendationsQuery(movieId, "movie");
  ...
  const getYoutubeVideo: (query: string) => Promise<void> = async (
    query: string
  ) => {
    setLoading(true);
    ...
    try {
      ...
      const video = data.items.find((movie: { snippet: { title: string } }) =>
        movie.snippet.title.startsWith(query)
      );
      ...
      // console.log(video)
      setYoutubeVideo(
        video.length > 1 ? video[0].id.videoId : data.items[0].id.videoId
      );
      setLoading(false);
  ...
  const storyWriters = credits?.crew.find((member: { job: string }) =>
    ["Story", "Writer"].includes(member.job)
  );
    ...
    >
      <Stack gap="5">
          ...
          >
            {clicked ? (
              ...
              <Stack
                ...
                onClick={() => getYoutubeVideo(details.title)}
                cursor="pointer"
                justifyContent="center"
                alignItems="center"
                bg="#00000040"
        ...
        >
            ...
            >
              ...
              {details.release_date && (
                <HStack>
                  ...
                  <Text fontSize={{ base: "", lg: "xl" }} fontWeight="bold">
                    {getNoOfDays(date)}
                  </Text>
                  <Text alignSelf="center">&#x2022;</Text>
                  <Text
              ...
              {storyWriters && storyWriters.length !== 0 && (
                <Stack>
                  <Text fontWeight="bold" fontSize={{ base: "lg", md: "xl" }}>
                    Story {storyWriters.length > 1 ? "Writers" : "Writer"}
                  </Text>
                  {storyWriters.length > 1 ? (
                    ...
                    <Text fontWeight="medium" fontSize={{ base: "", md: "lg" }}>
                      &#x2022; {storyWriters.original_name}
                    </Text>
                  )}
                </Stack>
              )}
            ...
            {topActors && (
              <Stack pt="5" gap="2" color={{ base: title, _dark: gray400 }}>
                ...
                <HStack
                  ...
                  pb={{
                    base: "",
                    md: recommendations?.results.length === 0 ? "5" : "",
                  }}
                >
                  {topActors.map(
                    (actor: {
          ...
          <Stack
            pb={{
              base: recommendations?.results.length === 0 ? "5" : "",
              md: "",
            }}
          >
        ...
        >
          <Stack gap={0}>
            ...
            <Text fontWeight="medium" fontSize={{ base: "md", lg: "lg" }}>
              Here are some movie recommendations based on this type of movie
            </Text>
          </Stack>
