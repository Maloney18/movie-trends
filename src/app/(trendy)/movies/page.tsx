...
const MoviesPage = () => {
  ...
  const getMovies = async ({ pageParam }: { pageParam: number }) => {
    try {
      const response = await fetch(
        `/api/tmdb/movies/popular?page=${pageParam}`
      );
  ...
  useEffect(() => {
    if (
      ...
      moviesPageScrollHeight &&
      moviesPageScrollHeight > 0
    ) {
  ...
  const movies = data?.pages?.map((page) =>
    page.results.map((movie: Tmovie, idx: number) => {
