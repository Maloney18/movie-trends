...
const SeasonDetails = ({ seasonInfo, setShowDetails }: incoming) => {
    ...
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/tv/${id}${
          seasonNo && seasonNo !== 0 ? "/season/" + seasonNo : "/season/0"
        }?language=en-US'`,
        {
          method: "GET",
          headers: {
  ...
  const { data } = useQuery({
    queryKey: ["seasonDetails", seasonInfo.seriesId, seasonInfo.seasonNumber],
    queryFn: () => getDetails(seasonInfo.seriesId, seasonInfo.seasonNumber),
  });
