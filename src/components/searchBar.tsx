...
const Searchbar = ({ color }: { color?: string }) => {
  ...
  const handleSearch: () => Promise<void> = async () => {
    ...
    try {
      ...
      const [movies, series] = await Promise.all([
        fetch(`/api/tmdb/search/movie?q=${search}`).then((r) => r.json()),
        fetch(`/api/tmdb/search/tv?q=${search}`).then((r) => r.json()),
  ...
  return (
    <Box
      ...
      pos="relative"
      alignSelf="center"
      w={{ base: "80%", md: "70%", lg: "525px" }}
      borderWidth="1px"
      borderColor={{ base: color || "white", _dark: "white" }}
      pl="2"
      pr="10"
      rounded="lg"
    >
      ...
      <Box
        ...
        cursor="pointer"
        className="group"
        onClick={() => search.length > 1 && handleSearch()}
      >
