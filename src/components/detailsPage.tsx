      ...
      <Stack gap='5'>
        ...
        <Flex direction={{base: 'column', lg: 'row'}} gap={{base: '10', md: ''}} pl='2' alignItems={{base: '', md:'start'}}>
          <Stack maxW={{base:'full', lg: '82%'}} flex='1' gap={{base: '2.5', md: ''}}>
            ...
            <HStack wrap='wrap'>
              {
                details.data.genres.map((genre: {id: string, name: string}) => (
                  <Text key={genre.id} borderWidth='1px' borderColor={{base:gameBg, _dark:rating}} color={{base:primaryColor, _dark: primaryLighter}} px='2' fontWeight='medium' rounded='3xl' fontSize={{base: 'xs', lg: 'md'}}>{genre.name}</Text>
                ))
              }
            </HStack>

            <Stack gap='1' mt='3' w={{base: '', lg:'70%'}}>
            {details.data.tagline && <Text color={{base: title, _dark: gray400}} fontSize={{base: '',md: 'md', lg: 'lg'}}>"{details.data.tagline}"</Text>}
              <Text color={{base:description, _dark: rating}} fontSize={{base: '', md: 'md', lg: 'lg'}}>{details.data.overview}</Text>
            </Stack>

            <Flex direction={{base: 'column', md: 'row'}} pr='5' justifyContent='space-between' alignItems='start' color={{base: title, _dark: gray400}} gap={{base: '5', md:'10'}} pt='2'>
              { 
              ...
              <Stack gap='2'>
                {
                  details.data.production_companies.map((company : {logo_path:string | null, id:string, origin_country: string, name: string}) => (
                      <Grid templateColumns={{base:'40px auto', md: '35px auto'}} key={company.id} gap='2' alignItems='center'>
                        <GridItem h={{base: '40px', md:'35px'}} w={{base: '40px', md:'35px'}} borderWidth='1px' overflow='hidden' rounded='full'>
        ...
        <Stack color={{base: title, _dark: gray400}} gap='7' pb='7' pl={{base: '2', md:''}}>
          <Stack gap={0}>
            <Flex direction={{base: 'column', md:'row'}} gap={{base: 0, md: '1.5'}}>
              <Text fontWeight='black' fontSize={{base: '2xl',md: '3xl', lg:'4xl'}}>{details.data.title ? "Movie" : "Series"}</Text>
              <Text fontWeight='black' fontSize={{base: '2xl',md: '3xl', lg:'4xl'}}>Recommendations</Text>
            </Flex>
            <Text fontWeight='medium' fontSize={{base: 'md', lg:'lg'}}>Here are some {details.data.title ? "movie" : "series"} recommendations based on this type of {details.data.title ? "movie" : "series"}</Text>
          </Stack>
