...
const MovieCard = ({date, title, genre, imgSrc, movieId, type} : incoming) => {
  ...
  return (
    <Grid cursor='pointer' gridTemplateRows={{base:'250px auto', md: '200px auto', lg: '250px auto'}} gap={3} w={{base:'200px', md: '150px', lg: '200px'}} onClick={() => router.push(`/${type === 'movie' ? 'movies' : 'series'}/${movieId}`)}>
      <GridItem h={{base:'250px', md: '200px', lg: '250px'}} w={{base:'200px', md: '150px', lg: '200px'}} bg={imgSrc ? '' : 'grey'} overflow='hidden' borderTopLeftRadius='lg' borderBottomRightRadius='lg'>
        {
          imgSrc && 
          <img 
            width='100%' 
